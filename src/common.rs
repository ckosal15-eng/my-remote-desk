...
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let url = "https://api.github.com/repos/ckosal15-eng/my-remote-desk/releases/latest";
    ...
    let latest_release_response = match client.get(url).header("User-Agent", "my-remote-desk").send().await {
        Ok(resp) => {
        ...
        Err(err) => {
            if is_tls_not_cached && err.is_request() {
                ...
                let client = create_http_client_async(tls_type, false);
                let resp = client.get(url).header("User-Agent", "my-remote-desk").send().await?;
                upsert_tls_cache(tls_url, tls_type, false);
                resp
            } else {
                return Err(err.into());
    ...
    let resp: GithubRelease = serde_json::from_slice(&bytes)?;
    let response_url = resp.html_url;
    let latest_release_version = response_url.rsplit('/').next().unwrap_or_default().trim_start_matches('v');
